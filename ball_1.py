...
def create_ball(scene_objects):
    """
    Standalone definition for Ball 1.

    Returns a dict with references to VPython objects:
    - sphere
    - string_left, string_right
    - anchors and attach points (useful later for physics)
    """
    rail_y = scene_objects["rail_y"]
    rail_front_z = scene_objects["rail_front_z"]
    rail_back_z = scene_objects["rail_back_z"]

    # Ball layout (keep identical across files; only x differs)
    ball_r = 0.035
    gap = ball_r * 0.06
